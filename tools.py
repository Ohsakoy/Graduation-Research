...
def dataset_split(images, labels, dataset='mnist', noise_type=None, noise_rate=0.5, split_per=0.9, random_seed=1, num_classes=10):
    ...
    if noise_type == 'symmetric':
        ...
        if dataset == 'linear':
            noisy_labels, noise_point= utils.noise_island_linear(images, labels_with_newaxis, noise_rate, -2.6, -0.2)
        ...
        if dataset == 'mnist':
            ...
            noisy_labels, noise_point = utils.noise_asymmetric_linear(labels_with_newaxis, noise_rate)
        elif dataset == 'nonlinear':
            noisy_labels, noise_point = utils.noise_asymmetric_nonlinear(
                labels_with_newaxis, noise_rate)
        elif dataset == 'covid_ct':
            noisy_labels, noise_point = utils.noise_asymmetric_covid(
                labels_with_newaxis, noise_rate)
    ...
    #print(np.where(clean_labels != noisy_labels)[0].shape)

...
parser.add_argument('--learning_rate', type=float, help='momentum', default=0.1)
...
if args.dataset == 'linear' or args.dataset == 'circle':
    ...
    noise_train_dataset = torch.utils.data.TensorDataset(train_X, train_y)
...
if args.method == 'CDR':
    ...
    noise_train_dataset = Noise_dataset_split(images_train, labels_train, train=True,
                                            transform=transform_train, target_transform=tools.transform_target,
    ...
    noise_val_dataset = Noise_dataset_split(images_train, labels_train, train=False,
                                            transform=transform_test, target_transform=tools.transform_target,
                                            noise_type=args.noise_type, noise_rate=args.noise_rate, dataset=args.dataset,
                                            split_per=SPLIT_TRAIN_VAL_RATIO, random_seed=1, num_class=num_classes)
    noise_val_loader = torch.utils.data.DataLoader(
        noise_val_dataset, batch_size=batch_size, drop_last=False, shuffle=False, num_workers=args.workers, pin_memory=True)
...
elif args.dataset == 'linear' or args.dataset == 'circle':
    clf = Logistic()
...
for epoch in range(args.start_epoch, args.epoch):
    ...
    if args.method == 'CDR':
        ...
        train_loss, train_acc = train(
            noise_train_loader,clf, device, criterion, optimizer)
    elif args.method == 'Trimming':
        train_loss, train_acc = trimming.train(noise_train_loader, clf, device, optimizer)
